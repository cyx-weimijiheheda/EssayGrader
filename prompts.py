...
# OCR文本中的无关信息（重要）
OCR识别的原始文本中可能混入了与作文正文无关的信息，例如：
- 学生个人信息栏（姓名、班级、学号、考号等标注文字或填写内容）
- 作文纸印刷的标题、说明文字、评分栏、页码等
- 试卷上的其他题目文字或答题提示

请在进行OCR修正时**识别并剥离这些无关信息**，使 ocr_corrected_text 字段**仅包含作文正文本身**。
- 姓名和班级信息请分别填入 student_name 和 student_class 字段，不要混入正文
- 其他无关信息（页码、评分栏、印刷标题等）直接丢弃，不出现在任何输出字段中
- 如果不确定某段文字是否为作文正文的一部分，优先保留

# 输出格式
请严格按照以下JSON格式输出，不要输出任何额外内容：
{
    "ocr_corrected_text": "仅修正OCR错误后的作文正文（已剥离个人信息和无关印刷文字）",
...
- **格式与语体（2分）**：应用文格式（如书信、通知、邮件等）是否基本正确，语气是否得体。注意：格式评分应宽松处理——书信末尾署名（如Yours sincerely / Li Hua）等格式细节如缺失不应单独扣分；重点关注应用文类型的整体格式框架（如称呼、分段）和语体是否恰当。
"""
...
4. **符合应用文格式规范**：确保书信/邮件/通知等的整体格式框架正确、语气得体（书信末尾署名等格式细节非必需，可酌情省略）
